...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    { name: "Locations", path: "/admin/locations", icon: "M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" },
    { name: "Rooms", path: "/admin/rooms", icon: "M5.25 14.25h13.5m-13.5 0a3 3 0 01-3-3m3 3a3 3 0 100 6h13.5a3 3 0 100-6m-16.5-3a3 3 0 013-3h13.5a3 3 0 013 3m-19.5 0a4.5 4.5 0 01.9-2.7L5.737 5.1a3.375 3.375 0 012.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 01.9 2.7" },
    { name: "All Bookings", path: "/admin/bookings", icon: "M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" },
  ];
  ...
  return (
    <div className="min-h-screen bg-black flex text-white font-sans">
          ...
          {links.map(link => {
            ...
            return (
              <Link 
                key={link.name} 
                href={link.path}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-all ${isActive ? 'bg-cinemac-blue text-white shadow-lg shadow-blue-500/20' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
              >
      ...
      {/* Main Content Area */}
      <main className="flex-1 bg-black overflow-y-auto w-full">
        {/* Mobile Header */}
        <header className="md:hidden h-16 border-b border-gray-800 flex items-center px-4 bg-gray-900 justify-between sticky top-0 z-30">
           <div className="flex items-center gap-3">
             ...
             <span className="font-black text-lg">CINEMAC<span className="text-cinemac-blue">ADMIN</span></span>
           </div>
                ...
                {links.map(link => {
                  ...
                  return (
                    <Link 
                      ...
                      className={`flex items-center gap-3 px-4 py-3 text-sm rounded-xl font-bold transition-all ${isActive ? 'bg-cinemac-blue text-white shadow-lg shadow-blue-500/20' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                        <path strokeLinecap="round" strokeLinejoin="round" d={link.icon} />
                      </svg>
